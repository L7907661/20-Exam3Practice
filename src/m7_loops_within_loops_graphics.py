...
def hourglass(window, n, point, radius, color):
    ...
    for k in range(n):
        for j in range(k + 1):
            aa = rg.Circle(rg.Point(xx,yy),r)
            aa.fill_color=color
            aa.attach_to(window)
            line1 = rg.Line(rg.Point(xx - r, yy), rg.Point(xx + r, yy))
            xx = xx + 2* r
...
def many_hourglasses(window, square, m, colors):
    ...
    kk = 0
    for k in range(m):
        rec = rg.Rectangle(rg.Point(xx, yy),rg.Point(x2, y2))
        kk = k
        if k > len(colors) - 1:
            kk = k - len(colors)
        if k > 2*len(colors) -1:
            kk = k - 2 *len(colors)
        hourglass(window, k + 1, rec.get_center(), r/2, colors[kk])
        rec.attach_to(window)
        xx = xx + r* (k + 1)
        yy = yy - r/2 *(math.sqrt(3))
        x2 = x2 + r * (k + 2)
        y2 = y2 + r/2 *(math.sqrt(3))
    window.render()
